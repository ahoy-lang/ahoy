...
Ahoy provides two keywords for controlling loop execution flow: `halt` and `next`.

## halt/halt Statement

The `halt` keyword immediately exits the innermost loop.
...
The `next` keyword nexts the rest of the current iteration and continues with the next iteration.
...
loop val in values do
    if val less_than 0 then
        next  ? next negative values
    print|"Valid: %d", val|
? Output: Valid: 10, 0, 15, 20
```

**next specific conditions:**
```ahoy
loop:0 to 20 do
    ? next multiples of 3
    if ? mod 3 is 0 then
...
You can use both `halt` and `next` together:
...
Both `halt` and `next` work with all loop types:
...
1. **Scope:** `halt` only exits the innermost loop it's contained in
2. **next vs Continue:** `next` is equivalent to `continue` in other languages
